...
public interface AggregationRepositoryBuilder {
     ...
     */
    AggregationRepository initWritable(Plugin plugin) throws Exception; //NOSONAR


    /**
     * Initialize the storage for plugin
     */
    <T> Storage<T> initStorage(Plugin plugin) throws Exception; //NOSONAR

    /**
     * Initialize the repository in readonly mode
     */
    <T> AggregatorRepository<T> initReadonly(Plugin plugin) throws Exception; //NOSONAR
}

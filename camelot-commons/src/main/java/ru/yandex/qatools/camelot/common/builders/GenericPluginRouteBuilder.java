...
public abstract class GenericPluginRouteBuilder extends RouteBuilder implements BasicRoutesBuilder {
    ...
    public GenericPluginRouteBuilder(Plugin plugin, CamelContext camelContext) throws Exception { //NOSONAR
    ...
    protected RouteDefinition appendSplitterRoutes(RouteDefinition route) throws Exception { //NOSONAR
        // Splitter (optional)
    ...
    protected RouteDefinition appendFilterRoutes(RouteDefinition route) {
        ...
        if (routeConfig.getFilterInstanceOf() != null) {
            addInterimProc(route.filter()
                    .method(new InstanceOfFilter(classLoader, plugin.getContext().getMessagesSerializer(),
                                    routeConfig.getFilterInstanceOf()), InstanceOfFilter.FILTER_METHOD_NAME))
    ...
    protected Expression splitExpression() throws Exception { //NOSONAR
    ...
    protected void startRoute(String id) throws Exception { //NOSONAR
        gracefullyStartRoute(camelContext, id);
    }

    protected void stopRoute(String id) throws Exception { //NOSONAR

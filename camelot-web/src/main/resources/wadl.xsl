...
<xsl:stylesheet version="2.0"
    ...
    <xsl:variable name="g_resourcesBase">
        <xsl:call-template name="getResourcesBase"/>
    </xsl:variable>

    <xsl:template name="getResourcesBase">
        <xsl:variable name="path" select="string(wadl:application/wadl:resources/@base)"/>
        <!--<xsl:value-of select="replace($path, string('https?://[^/]+(.*)'), string('$1'))" disable-output-escaping="yes" />-->
        <xsl:value-of select="wadl:application/wadl:resources/@base" disable-output-escaping="yes" />
    </xsl:template>

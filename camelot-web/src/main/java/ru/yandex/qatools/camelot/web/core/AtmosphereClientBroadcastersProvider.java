...
import ru.yandex.qatools.camelot.web.jackson.JsonSerializer;
...
public class AtmosphereClientBroadcastersProvider implements LocalClientBroadcastersProvider {
    ...
    @Override
    public synchronized Broadcaster getBroadcaster(String topic) throws Exception { //NOSONAR
        try {
            if (!enabled) {
                throw new PluginsSystemException("Failed to retrieve broadcaster: provider is disabled!");
            }
            topic = (isEmpty(topic)) ? "" : topic; //NOSONAR
    ...
    private PluginBroadcaster initBroadcaster(String broadcasterId) throws UnsupportedEncodingException {
        ...
        if (framework == null) {
            throw new RuntimeException("Failed to init broadcaster: application context is " + //NOSONAR
                    "probably not web application context!");
        }

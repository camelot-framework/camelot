...
import ru.yandex.qatools.camelot.core.impl.PluginAnnotatedMethodInvoker;
...
public class QuartzAggregatorSchedulerBuilder implements SchedulerBuilder {
        ...
        if (schedule.matches(PROP_EXPR)) {
            ...
            try {
                ...
                for (Method m : getMethodsInClassHierarchy(pClazz)) {
                    if (m.getName().equals(cronMethod)) {
    ...
    @Override
    public boolean invokeJob(String methodName) throws Exception {

...
import ru.yandex.qatools.camelot.core.MetadataClassInfo;
...
public class AnnotatedMethodDispatcher {
    ...
    public Map<Method, Object> dispatch(Class<? extends Annotation> annClass, boolean singleCall, Object... params) throws Exception {
        ...
        for (int i = 0; i <= paramTypes.size(); ++i) {
            for (int j = paramTypes.size(); j >= i; --j) {
                ...
                final Stack<Class[]> typesStack = new Stack<>();
    ...
    protected void call(Class<? extends Annotation> annClass, Stack<Class[]> typesStack, List<Object> params,
                     boolean singleCall, Map<Method, Object> called, int paramIdx) throws Exception {
        try {
            if (singleCall && !called.isEmpty()) {
                return;
            }
    ...
    private Exception wrapThrowableIfRequired(Throwable e, Throwable cause) throws Exception {
        if (cause == null) {
    ...
    private void findSuitableMethodAndCall(Class<? extends Annotation> annClass, List<Class> paramTypes, List<Object> params,
                                           boolean singleCall, Map<Method, Object> called) throws Exception {
//        for (Class type : paramTypes) {
//            System.out.print(type.getSimpleName() + ",");
//        }
//        System.out.println("");
        final Class[] mParamTypes = paramTypes.toArray(new Class[paramTypes.size()]);
        final Collection<Method> methods = (Collection<Method>) cache.getMethodsByParamTypes(annClass, mParamTypes);
